...
use max only N of sparks when there are more than one source states and never merge until the end of computation
...
> module Text.Regex.PDeriv.ByteString.LeftToRightP5
>     ( Regex
>     , CompOption(..)
>     , ExecOption(..)
>     , defaultCompOpt
...
> collectPatMatchFromBinder_ w ((x,[]):xs) = 
>   let  m =  (x,S.empty)
>        ms = collectPatMatchFromBinder_ w xs
>   in {- m `par` ms `seq` -} (m:ms) 
> collectPatMatchFromBinder_ w ((x,rs):xs) = 
>   let m = (x,foldl' S.append S.empty $ map (rg_collect w) (reverse rs))
>       ms = collectPatMatchFromBinder_ w xs
>   in m `par` ms `seq` (m:ms)
>  -}
...
> 
> collectPatMatchFromBinder_ w [] = []
> collectPatMatchFromBinder_ w ((x,[]):xs) = runEval $ do 
>          { m  <- Control.Parallel.Strategies.rpar (x,S.empty)
>          ; ms <- Control.Parallel.Strategies.rseq (collectPatMatchFromBinder_ w xs)
>          ; return (m:ms) }
> collectPatMatchFromBinder_ w ((x,rs):xs) = runEval $ do 
>          { m      <- do 
>             { mapped <- parList Control.Parallel.Strategies.rpar $ map (rg_collect w) (reverse rs)
>             ; return (x,foldl' S.append S.empty  mapped) }
>          ; ms     <- Control.Parallel.Strategies.rseq (collectPatMatchFromBinder_ w xs)
>          ; return (m:ms) }
> 
...
>   | length eps > 1 = 
>       let epss = parMap rpar (\ep -> patMatchesIntStatePdPat0Par cnt pdStateTable w' [ep]) eps            
>       in epss `seq` nub2 $ concat epss
>   | otherwise =  
...
> patMatchesIntStatePdPat0Par :: Int -> PdPat0Table -> Word -> [(Int,Binder)] -> [(Int,Binder)]
> patMatchesIntStatePdPat0Par cnt pdStateTable  w' eps =
>     case {-# SCC "uncons" #-} S.uncons w' of 
>       Nothing -> eps 
>       Just (l,w) -> 
>           let 
...
>     -- all_func = all_func' `seq` map snd (filter (\(i,_) -> i `elem` sfinal) all_func' ) 
>   in map (collectPatMatchFromBinder w) allbinders
>      -- no need to par here, as we only need the first one.
>      {- all_func `seq` 
